...
#[derive(Clone)] pub struct AppState { pub pool:SqlitePool, pub sources:source::SharedSourceEngine, pub bus:Arc<events::EventBus> }
async fn health()->impl IntoResponse{Json(json!({"status":"ok","version":"3.0.0","api":"v1"}))}
async fn list_nodes(State(s):State<Arc<AppState>>)->impl IntoResponse{match repository::list_nodes(&s.pool,100).await{Ok(v)=>(StatusCode::OK,Json(json!({"items":v}))).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}
...
async fn export_nodes(State(s):State<Arc<AppState>>)->impl IntoResponse{match repository::list_nodes(&s.pool,1000).await{Ok(v)=>(StatusCode::OK,[(axum::http::header::CONTENT_TYPE,"application/json")],export::json(&v)).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}
async fn qr_node(State(s):State<Arc<AppState>>,Path(id):Path<Uuid>)->impl IntoResponse{match repository::list_nodes(&s.pool,1000).await{Ok(v)=>match v.into_iter().find(|n|n.id==id){Some(n)=>match export::qr_ascii(&n){Ok(q)=>(StatusCode::OK,[(axum::http::header::CONTENT_TYPE,"text/plain; charset=utf-8")],q).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()},None=>(StatusCode::NOT_FOUND,Json(json!({"error":"node not found"}))).into_response()},Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}
...
async fn test_node(State(s):State<Arc<AppState>>,Path(id):Path<Uuid>)->impl IntoResponse{match repository::list_nodes(&s.pool,1000).await{Ok(v)=>match v.into_iter().find(|n|n.id==id){Some(n)=>match tester::tcp(&s.pool,&n,5000).await{Ok(t)=>{s.bus.publish(events::AppEvent::NodeTested{node_id:id.to_string(),success:t.success,latency_ms:t.latency_ms});(StatusCode::OK,Json(t)).into_response()},Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()},None=>(StatusCode::NOT_FOUND,Json(json!({"error":"node not found"}))).into_response()},Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}
async fn create_job(State(s):State<Arc<AppState>>)->impl IntoResponse{match jobs::create_job(&s.pool,&s.bus,"manual").await{Ok(id)=>(StatusCode::ACCEPTED,Json(json!({"job_id":id,"status":"queued"}))).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}

pub async fn router(database_url:&str)->anyhow::Result<Router>{let pool=db::connect(database_url).await?;let fetcher=source_fetcher::SourceFetcher::new(env::var("NODEEASY_ALLOW_PRIVATE_SOURCES").ok().as_deref()==Some("true"))?;let engine=Arc::new(source::SourceEngine::new(pool.clone(),fetcher));let bus=Arc::new(events::EventBus::new(512));let state=Arc::new(AppState{pool,sources:engine,bus});Ok(Router::new().route("/api/v1/health",get(health)).route("/api/v1/nodes",get(list_nodes)).route("/api/v1/nodes/{id}/test",post(test_node)).route("/api/v1/sources",get(list_sources).post(create_source)).route("/api/v1/sources/{id}/sync",post(sync_source)).route("/api/v1/jobs",post(create_job)).route("/api/v1/export/nodes.json",get(export_nodes)).route("/api/v1/nodes/{id}/qr",get(qr_node)).route("/api/v1/ws",get(ws::handler)).with_state(state))}
