...
async fn export_nodes(State(state):State<Arc<AppState>>)->impl IntoResponse{match repository::list_nodes(&state.pool,1000).await{Ok(nodes)=>(StatusCode::OK,[(axum::http::header::CONTENT_TYPE,"application/json")],export::json(&nodes)).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}
async fn qr_node(State(state):State<Arc<AppState>>,Path(id):Path<Uuid>)->impl IntoResponse{match repository::list_nodes(&state.pool,1000).await{Ok(nodes)=>match nodes.into_iter().find(|n|n.id==id){Some(node)=>match export::qr_ascii(&node){Ok(qr)=>(StatusCode::OK,[(axum::http::header::CONTENT_TYPE,"text/plain; charset=utf-8")],qr).into_response(),Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()},None=>(StatusCode::NOT_FOUND,Json(json!({"error":"node not found"}))).into_response()},Err(e)=>(StatusCode::INTERNAL_SERVER_ERROR,Json(json!({"error":e.to_string()}))).into_response()}}
