...
impl Node {
    pub fn fingerprint(protocol: Protocol, endpoint: &NodeEndpoint, identity: &[(&str, &str)]) -> String {
        let mut hasher = Sha256::new();
        hasher.update(format!("{:?}|{}|{}", protocol, endpoint.host.to_ascii_lowercase(), endpoint.port));
...
pub enum CoreError {
    ...
    #[error("source blocked by SSRF policy")]
    BlockedSource,
